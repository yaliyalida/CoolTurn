...
def batch_write(filenum,fieldnum):
    ...
    for i in range(fieldnum):  # 逐一遍历字段
        ...
        fields.append(files)
        # print("已存储第{}个字段对应的每一份文件内容".format(i+1))
    return fields

def write_words(template,specific_chrt,fields,filenum,name_rules=0):
    '''
...
if __name__ == "__main__":
    formatpath = 'template.docx'  # 模板文件路径
    template = get_tplt(formatpath)
    print("模板内容为:{}".format(template),end='')
    ...
    write_words(template, specific_chrt, fields, filenum, name_rules)
